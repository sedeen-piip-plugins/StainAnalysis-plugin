...
void StainAnalysis::init(const image::ImageHandle& image) {
	...
	auto color = getColor(image);
	auto max_value = (1 << color.bitsPerSample()) - 1;
	m_threshold = 
...
std::string StainAnalysis::generateReport() const{
	...
	if(full_size.width() > output_image.width()){
		...
		_numPixels = (float)(numPixels*scalew*scaleh)/(float)_totalnumPixels;
